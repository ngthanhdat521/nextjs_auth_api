...
declare global {
  interface Window {
    FlowbiteInstances: {
      getInstance: (component: string, id: string) => any;
...
export function create() {
  if (document.getElementById('wysiwyg-example')) {
    ...
    const wysiwyg = document.querySelector('#wysiwyg-example');
    ...
    // typography dropdown
    const typographyDropdown = window.FlowbiteInstances.getInstance(
      'Dropdown',
      'typographyDropdown'
    );
    ...
    const textSizeDropdown = window.FlowbiteInstances.getInstance(
      'Dropdown',
      'textSizeDropdown'
    );
    ...
    if (colorPicker) {
      colorPicker?.addEventListener('input', (event) => {
        const selectedColor = (event.currentTarget as HTMLInputElement).value;
    ...
    const fontFamilyDropdown = window.FlowbiteInstances.getInstance(
      'Dropdown',
      'fontFamilyDropdown'
    );
